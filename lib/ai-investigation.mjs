...
export function investigationNeedsRefresh(record, nowMs = Date.now(), { verifiedMaxAgeMs = 20 * 60_000, retryAfterMs = 5 * 60_000 } = {}) {
  if (!record?.created_at) return true;
  const createdAt = Date.parse(record.created_at);
  if (!Number.isFinite(createdAt)) return true;
  const age = Math.max(0, nowMs - createdAt);
  return Number(record.valid) === 1 ? age >= verifiedMaxAgeMs : age >= retryAfterMs;
}
