...
export function parseServicerFeed(input) {
  ...
  const tokenSupply = Number(snapshot.tokenSupply);
  if (!Number.isFinite(tokenSupply) || tokenSupply <= 0) throw new Error("snapshot.tokenSupply must be a positive number.");
  ...
  return {
    ...
    signature,
    snapshot: { poolId, capturedAt, heartbeat, tokenSupply, source: requiredString(snapshot.source || "servicer", "snapshot.source") },
    assets,
    payments,
  };
}
