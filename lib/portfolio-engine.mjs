...
export function analyzePortfolio(portfolio) {
  ...
  const assessedAssets = assets.map((asset) => {
    ...
    const eligible = !duplicate && !overdue && !stale && asset.status !== "repaid";
    return { ...asset, daysPastDue: Math.max(0, daysPastDue || 0), freshnessDays, duplicate, eligible };
  });
  ...
  return {
    ...
    asOf,
    state,
    metrics: { assetCount: assets.length, totalFaceValue, totalOutstanding, eligibleOutstanding, grossCoverage, eligibleCoverage, highAlerts, mediumAlerts },
...
export function parseAssetTapeCsv(text, defaults = {}) {
  ...
  return { poolId: defaults.poolId || "IMPORTED-POOL", poolName: defaults.poolName || "Imported receivables pool", asOf: defaults.asOf || new Date().toISOString(), tokenSupply: number(defaults.tokenSupply) || assets.reduce((sum, asset) => sum + number(asset.outstanding), 0), assets };
}
