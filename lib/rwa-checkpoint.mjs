...
export function buildRwaCheckpoint({ projectId, chainId, contractAddress, fromBlock, toBlock, confirmationBlock, finalityStatus = "CONFIRMED", confirmationDepth = null, rpcUrl = null, events = [], previousAccounts = {}, previousRedemptions = [] }) {
  ...
  if (!["PRECONFIRMED", "CONFIRMED", "FINALIZED"].includes(String(finalityStatus))) throw new Error("Checkpoint finality status is invalid.");
  if (confirmationDepth != null && !/^\d+$/.test(String(confirmationDepth))) throw new Error("Checkpoint confirmation depth is invalid.");
  ...
  for (const previous of previousRedemptions) {
    const requestId = String(previous?.requestId || "");
    const previousAccount = String(previous?.account || "").toLowerCase();
    const previousAmount = String(previous?.amount || "");
    const status = String(previous?.status || "QUEUED").toUpperCase();
    if (!requestId || !/^0x[0-9a-f]{40}$/.test(previousAccount) || !/^\d+$/.test(previousAmount) || !["QUEUED", "SETTLED"].includes(status)) throw new Error("Previous redemption state is invalid.");
    if (redemptions.has(requestId)) throw new Error("Duplicate previous redemption request.");
    redemptions.set(requestId, { requestId, account: previousAccount, amount: previousAmount, status });
  }
  ...
  const confirmedEvents = ordered.filter((event) => BigInt(event.blockNumber) >= BigInt(fromBlock) && BigInt(event.blockNumber) <= BigInt(toBlock));
  ...
  const eventRows = confirmedEvents.map((event) => ({ id: `${event.transactionHash}:${event.logIndex}`, event: event.event, blockNumber: String(event.blockNumber), blockHash: event.blockHash || null, args: event.args }));
  const checkpoint = { schema: "duevia.rwa-checkpoint/v1", projectId, chainId: Number(chainId), contractAddress: contractAddress.toLowerCase(), fromBlock: String(fromBlock), toBlock: String(toBlock), confirmationBlock: String(confirmationBlock), finalityStatus: String(finalityStatus), confirmationDepth: confirmationDepth == null ? null : String(confirmationDepth), rpcUrl: typeof rpcUrl === "string" && rpcUrl ? rpcUrl : null, assetStateRoot: root(eventRows), accountBalanceRoot: root(accountRows.map(({ account, principal }) => ({ account, principal }))), yieldStateRoot: root(accountRows.map(({ account, yield: yieldAmount }) => ({ account, yield: yieldAmount }))), redemptionQueueRoot: root(redemptionRows), evidenceRoot: root(eventRows.map(({ id, blockHash }) => ({ id, blockHash }))), accounts: accountRows, redemptions: redemptionRows, eventCount: eventRows.length };
