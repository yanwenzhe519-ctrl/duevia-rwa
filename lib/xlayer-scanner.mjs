...
export async function scanXLayer({ rpcUrl = XLAYER_TESTNET_RPC, fromBlock, toBlock, addresses = [], fetchImpl = fetch } = {}) {
  ...
  const end = toBlock == null ? latest : BigInt(toBlock);
  const start = fromBlock == null ? (end > DEFAULT_SCAN_DEPTH ? end - DEFAULT_SCAN_DEPTH : 0n) : BigInt(fromBlock);
  ...
  return { chainId: 1952, rpcUrl, fromBlock: start.toString(), toBlock: end.toString(), latestBlock: latest.toString(), observationCount: observations.length, projects, observations };
}
