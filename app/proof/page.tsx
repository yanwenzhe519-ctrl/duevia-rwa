...
import { dueviaContracts, dueviaFinalRehearsal, dueviaGovernanceTransactions, legacyRehearsalTransactions } from "@/lib/deployment-evidence";
...
const evidence = [
  ...
  ...legacyRehearsalTransactions.map((item) => ({ label: item.label, value: `${item.transaction.slice(0, 12)}...${item.transaction.slice(-6)}`, href: `${explorer}/tx/${item.transaction}`, note: "Historical enforcement proof; separate from final-stack rehearsal" })),
  ...dueviaFinalRehearsal.transactions.map((item) => ({ label: item.label, value: `${item.transaction.slice(0, 12)}...${item.transaction.slice(-6)}`, href: `${explorer}/tx/${item.transaction}`, note: `Final-stack rehearsal · block ${item.blockNumber}` })),
];
...
export default function ProofPage() {
  return <main className="proof-page">
    ...
    <section className="proof-runtime"><div className="shell proof-runtime-grid"><div><span className="section-index light">04 / LIVE RUNTIME</span><h2>Persistent monitoring,<br />not a local demo.</h2></div><div className="runtime-cards"><a href="/api/agent/health" target="_blank" rel="noreferrer"><span>AI investigation</span><b>Workers AI</b><small>Structured output and independent verifier</small></a><a href="/api/watchdog" target="_blank" rel="noreferrer"><span>Continuity watchdog</span><b>D1 + Cron</b><small>X Layer scanner and persistent incident metadata</small></a><a href="/api/recovery" target="_blank" rel="noreferrer"><span>Recovery archive</span><b>Capsule roots</b><small>Public metadata; raw recovery data remains protected</small></a><a href="/api/evidence" target="_blank" rel="noreferrer"><span>Generated evidence</span><b>D1 evidence/v2</b><small>Release, contracts, incidents, roots, blocks, and Keeper runs</small></a></div></div></section>

    <section className="proof-section shell proof-status">
      <div className="proof-heading"><div><span className="section-index">05 / DELIVERY STATUS</span><h2>Verified versus pending</h2></div><p>The final testnet stack and multisig ownership are deployed. Automatic broadcasting remains disabled by design; every material transition requires explicit authorization.</p></div>
      <div className="status-grid"><article className="status-complete"><span>VERIFIED NOW</span><h3>Final-stack incident rehearsal completed from SUSPENDED through successor VERIFIED and an accepted 1 wei deposit.</h3><p>Recovery Root, Incident ID, linked attestations, Pool rejection, and all seven final rehearsal transactions are published above and in the machine-readable evidence API.</p></article><article className="status-complete"><span>LIVE CONTROLS</span><h3>AI, deterministic reconstruction, persistent Keeper, failover, public code, and all six final X Layer Testnet contracts.</h3><p>Registry and Coordinator ownership resolve to the Recovery Multisig, while Observer Quorum and Recovery Multisig are authorized project operators.</p></article><article className="status-pending"><span>BEFORE MAINNET</span><h3>External contract audit, independent operator organizations, real RWA data adapters, pilot partner, and governance/legal operating controls.</h3><p>Only after these controls exist can automatic SUSPENDED broadcast be considered. It is not enabled today.</p></article></div>
    </section>
