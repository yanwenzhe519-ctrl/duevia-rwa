...
export default function InfrastructureDeployer({ wallet, registryAddress }: { wallet: string; registryAddress: string }) {
  ...
  return <div className="infrastructure-panel"><div className="panel-title"><div><span>X LAYER INFRASTRUCTURE</span><h3>Autonomous continuity runtime</h3></div><b>{runtime.persistent && runtime.model === "model-grounded" ? "LIVE" : "PARTIAL"}</b></div><div className="monitor-grid"><div className="monitor-card"><span>Persistent Watchdog</span><strong>{runtime.persistent ? "D1 LIVE" : "OFFLINE"}</strong><small>{runtime.keeperRuns} recent keeper run(s)</small></div><div className="monitor-card"><span>AI investigator</span><strong>{runtime.model === "model-grounded" ? "MODEL LIVE" : "FALLBACK"}</strong><small>Structured output + verifier</small></div><div className="monitor-card"><span>Recovery state</span><strong>{runtime.incidents} incidents</strong><small>{runtime.capsules} capsules · {runtime.queuedActions} approval queue</small></div><div className="monitor-card"><span>Coordinator</span><strong>{coordinator ? short(coordinator) : "NOT DEPLOYED"}</strong><small>X Layer Testnet</small></div></div><div className="continuity-buttons"><button className="upload-package" type="button" onClick={deployCoordinator} disabled={running || Boolean(coordinator)}>{coordinator ? "Coordinator deployed" : "Deploy Coordinator"}</button><button className="upload-package" type="button" onClick={deployGuard} disabled={running || !coordinator || !registryAddress || Boolean(continuityGuard)}>{continuityGuard ? "Dual Guard deployed" : "Deploy Dual Guard"}</button><button className="anchor-button" type="button" onClick={deployPool} disabled={running || !continuityGuard || Boolean(continuityPool)}>{continuityPool ? "Continuity Pool deployed" : "Deploy Continuity Pool"}</button></div><small className="proof-note">{notice}</small></div>;
}
