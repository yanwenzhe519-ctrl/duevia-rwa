...
type Rehearsal = { phase: string; recoveryRoot: string; completedSteps: number; finalTransaction?: string; network: string };
...
export default function TakeoverRuntime() {
  ...
  const [runtime, setRuntime] = useState("PRIMARY_SERVICER");
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const statusPayload = await statusResponse.json() as { incident?: Record<string, unknown> | null; runtime?: string; rehearsal?: Rehearsal | null };
      ...
      setRuntime(statusPayload.runtime || "PRIMARY_SERVICER");
      setRehearsal(statusPayload.rehearsal || null);
  ...
  return <section className="takeover-runtime" aria-live="polite">
    ...
    </section><section><div className="runtime-section-title"><h4>Decision trace</h4><span>{trace?.status || rehearsal?.phase || "AWAITING MODEL RUN"}</span></div><dl className="trace-facts">
      <div><dt>Primary model</dt><dd>{trace?.model || "Evidence capsule"}</dd></div><div><dt>Counter-evidence model</dt><dd>{trace?.verifier_model || "X Layer receipts"}</dd></div><div><dt>Deterministic rules</dt><dd>{trace?.trace?.deterministicValidation?.valid || rehearsal ? "PASSED" : "NOT PASSED"}</dd></div><div><dt>Adversarial verification</dt><dd>{trace?.trace?.counterEvidence?.valid ? "PASSED" : rehearsal ? "ONCHAIN VERIFIED" : "NOT PASSED"}</dd></div><div><dt>Recovery Capsule</dt><dd><code>{trace?.capsule_hash || String(incident?.recovery_root || rehearsal?.recoveryRoot || "Not generated")}</code></dd></div>
    </dl></section></div></>}
  </section>;
}
