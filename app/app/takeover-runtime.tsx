...
export default function TakeoverRuntime() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [statusResponse, checkpointResponse, redemptionResponse, traceResponse] = await Promise.all([fetch(`${root}/status`, { cache: "no-store" }), fetch(`${root}/checkpoints`, { cache: "no-store" }), fetch(`${root}/redemptions`, { cache: "no-store" }), fetch(`${root}/decision-trace`, { cache: "no-store" })]);
      if (![statusResponse, checkpointResponse, redemptionResponse, traceResponse].every((response) => response.ok)) throw new Error("The takeover runtime is not initialized. Apply D1 migration 0008 before enabling this panel.");
