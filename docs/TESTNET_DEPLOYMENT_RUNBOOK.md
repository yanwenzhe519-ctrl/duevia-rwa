...
The public AI health endpoint must continue to report `DEGRADED` / `review-required` when validation fails. The historical testnet record failed because malformed model JSON produced no cited facts; the primary structured call now has three bounded attempts, and the verifier uses the current catalog model `@cf/meta/llama-3.1-8b-instruct-fp8`. The verifier prompt explicitly treats `requiresApproval: true` as an approval gate rather than an absent approval. Keeper retries a failed investigation for the same incident and recovery root after a five-minute cooldown, using numeric validation semantics so D1 `0` cannot be mistaken for a truthy string. Persisted health becomes `READY` only after a retry passes both deterministic and independent verification. These are evidence-quality controls, not wallet errors, and no validated candidate is fabricated.
