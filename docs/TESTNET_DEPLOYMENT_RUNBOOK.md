...
The public AI health endpoint must continue to report `DEGRADED` / `review-required` when validation fails. The historical testnet record failed because malformed model JSON produced no cited facts; the primary structured call now has three bounded attempts. The verifier uses the current catalog model `@cf/meta/llama-3.1-8b-instruct-fp8`, but requests plain JSON rather than unsupported model-side JSON Schema and then applies strict local parsing and field validation. The verifier prompt explicitly treats `requiresApproval: true` as an approval gate rather than an absent approval. Keeper retries a failed investigation for the same incident and recovery root after a five-minute cooldown, using numeric validation semantics so D1 `0` cannot be mistaken for a truthy string. Persisted health becomes `READY` only after a retry passes both deterministic and independent verification. These are evidence-quality controls, not wallet errors, and no validated candidate is fabricated.
...
Before any testnet write, verify the five takeover contracts and their roles from `/api/evidence` plus read-only RPC. Recovery Multisig role ownership and bootstrap-role revocation are RPC-verified for the takeover runtime; business registration and project-specific evidence remain separate verification layers. Workers AI is only `model-grounded` when that project's own evidence capsule passes deterministic and independent verification; otherwise it remains `review-required`. Do not rewrite historical `UNKNOWN` finality statuses.
