...
1. **Observation plane**: a bounded X Layer JSON-RPC scanner discovers Duevia protocol events across all emitting addresses; signed, allowlisted observer adapters and public reporting provide independent offchain signals. Generic HTTP uptime never refreshes a trusted servicing heartbeat.
...
2. `POST /api/servicer-feed` verifies the schema, HMAC signature, freshness, registered pool, and a D1-persisted replay key.
...
`GET /api/operations/health` evaluates the five-minute Keeper SLO from persistent run history, lease state, scanner freshness, and trigger-source diversity. A separately operated authenticated Keeper can contend for the same D1 lease; redundancy is not reported as verified until both primary and secondary trigger identities have completed successful runs.
